...
import {
  ...
  getSessionTypeKind,
  getSeatsStatusText,
} from "../utils/sessionTypeOptionUtils";
...
const SessionTypeCard = ({
  ...
  const isUnavailable =
    isFullyBooked || option.isAvailable === false || option.available === false;
  const hasLimitedSeats =
    !isUnavailable && option.availableSeats > 0 && option.availableSeats < 5;
  ...
  return (
    <div className="w-[171.3px] h-[154px] flex flex-col gap-[8px]">
      <button
        ...
        className={`text-center w-full h-[131px] rounded-[12px] px-[20px] py-[15px] flex flex-col items-center border ${
          isUnavailable
            ...
            : isSelected
              ? "border-[#958FEF] bg-[#DDDBFA] cursor-pointer"
      ...
      >
        ...
        />
        <h3
          className={`my-[6px] w-[131.33px] h-[19px] font-[600] leading-[100%] ${textColor}`}
        >
          {sessionTypeName}
        </h3>
        <div
          className={`w-[131.3px] h-[15px] font-[400] text-[12px] leading-[100%] flex flex-row items-center justify-center gap-[4px] ${textColor}`}
        >
          <LOCATION
            aria-hidden
